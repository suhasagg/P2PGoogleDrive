...
#Networking layer is implemented from scratch
#Advantages of the design
#P2P layer framework to implement sender
class Sender:
    ...
    def get_field(self, decoded_msg, flags = False, seq = False, ack = False, con_num = False, checksum = False, ad_win = False, data = False):
        if flags:
    ...
    # send <1460 at once
    # Chunking can be useful in below domains -
    # Dropbox - chunking algorithm as per dropbox design (to reduce data transfer, deduplication checks)
    # increase efficiency of transmission
    # Recieve a file chunk - further splits to packets
    def mrt_send(self,msg):
        # check to see if can send, and wait if cannot
        # add new stuff to send to list
        # integrate custom p2p layer framework (custom p2p network topology - Dropbox p2p based framework - Full custom event logging (Full file exchange detail logs)
        ...
        for packs in new_packs:
            ...
            if(self.get_field(packs,seq=True) > self.updated_base and self.get_field(packs,seq=True) + len(packs) < available_window):
    ...
    def resend_all(self):
        ...
        for i in range(len(self.unacked)):#pack in self.unacked:
            pack = self.unacked[i]
            available_window = self.recv_window - self.sent_bytes
            if(self.get_field(pack,seq=True) > self.updated_base and len(pack) < available_window):
    ...
    def process_data(self,packet):
        ...
        if(self.check_packet(packet)):
            ...
            if(self.get_field(packet,flags=True) == "100"):
                ...
                # cumulative acks, so if got it, can remove all packets w sequen e numbers
                # below
                max_base = 0
                for pack in self.unacked:
                    if self.get_field(pack,seq=True) == ack_num-1 and max_base < ack_num:
                        max_base= ack_num 
            ...
            if time.time() - self.buffer_time > 5:
                ...
                for packet in self.unacked:
                    if self.get_field(packet,seq=True) > self.updated_base:
                        temp.append(packet)
                self.unacked = temp
